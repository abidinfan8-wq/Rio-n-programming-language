#!/usr/bin/env python3
"""
RIO v5 - لغة مستقلة بنحو خاص كامل (بدون تمرير بايثون خام)
================================================================
...
لا يوجد "تمرير خام" لأي سطر - كل شيء يُفهم صراحة أو يُرفض بخطأ واضح.
    ...
    المتغيرات:    VAR = EXPR   /   set VAR = EXPR (توافق قديم)
    ...
    أخرى:        set / input / import / from import / draw (كما بالإصدار السابق)
"""
...
def transpile_lines(source: str, base_dir: str, visited: set) -> tuple:
    ...
    for raw_idx, raw in enumerate(lines):
        ...
        # ---- cls Name: ----
        m = re.match(rf"cls\s+({IDENT_RE})\s*:\s*$", stripped, re.IGNORECASE)
        if m:
            put(f"class {m.group(1)}:", push="cls")
            continue
        ...
        if m:
            put(f"{m.group(1)} = {stripped[len('set '+m.group(1)):].split('=',1)[1].strip()}"
                if False else f"{m.group(1)} = {m.group(2)}")
            continue
